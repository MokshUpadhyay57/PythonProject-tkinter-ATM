...
def Transfer():
    ...
    if(row>0):
        ...
        query = "update statebank set Amount = Amount + %s where Account = %s and IFSC = %s"
        values = (amnt,ac,code)
        a.execute(query,values)
        # a.execute("update statebank set amount=amount+'"+str(amnt)+"' where account='"+ac+"' and ifsc='"+code+"' ")
